...
The following task planning flow chart describes the combined operation of the Clamps and Screwdrivers. It is the result of merging the two previous flow charts \seerefii{subsubsection:exploration_3_task_planning_with_flowchart}{subsection:exploration_4_flowchart_for_screwdriver_assembly}.
...
In the previous planning workflow \seeref{subsection:exploration_3_process_design_workflow} tasks were planned using the flowchart method \seeref{subsubsection:exploration_3_task_planning_with_flowchart} without considering the geometrical constraints, such as reachability and collision. Motion planning happens only at a later stage with the already fixed task list. 
...
The polyline lap joint was used as the parametric model that describes a design space \seeref{subsection:exploration_4_parametric_polyline_lap_joints}. Fabrication checks were performed during the joint design process to ensure that the results did not violate the 90-degree rule \seeref{subsection:exploration_4_cnc_limitation_in_cutting_lap_joints}. 
...
The deformation-aware design principle was applied to the design of the structure \seeref{subsection:exploration_4_deformation_awareness_and_error_correction_by_triangulation}. However, because the reciprocal structural system was the design focus, and diagonal bracing would have defeated the idea. Therefore, the previous error correction method of adding diagonal bracings elements was not used.
...
In order to avoid collision during the robotic operation \seeref{subsection:exploration_3_robot_collision_with_scaffolding}, the estimated size of the scaffolding bar and the carpentry clamps are added to the 3D model as collision geometry for motion planning.
...
After the implementation of the vision-guided docking procedure \seeref{subsection:exploration_4_camera_marker_hardware_on_docking_adapter} and the compliant robotic arm control \seeref{subsection:exploration_4_compliant_control_for_robotic_arm}, there are still cases where the docking adapter failed to lock \seeref{subsection:exploration_3_docking_adapter_fail_to_lock}.
...
\begin{itemize}
	\item \textbf{Initial State --} Identical to the initial state used for sequential planning \seeref{subsubsection:exploration_3_compute_object_states}. All the state parameters of all objects are required.
...
\begin{itemize}
	...
	\item \textbf{Effects --} A symbolic change to the scene state if that action is taken. This is conceptually the same as the \verb{task.UpdateState()} function \seeref{subsubsection:exploration_3_compute_object_states} but implemented for the High-Level Tasks.
...
\begin{itemize}
	\item IK solvers have access to the same Robot Model and Tool Models \seeref{subsection:exploration_3_process_design_workflow} for checking IK and collisions.
...
\end{itemize}
	...
	\item \textbf{Execution --} The execution of the planned tasks and motion on the robot is the same as before \seeref{subsection:exploration_3_standalone_process_execution_controller}. One side effect is that the planned tasks no longer correspond to a specific beam in a sequential way, but this does not affect operations.
...
The preparation of the execution was similar to the previous round \seeref{subsection:exploration_4_execution_plan}. The differences are:
...
The polyline lap joints used in this round are slightly more complex than the ones used in the HyparHut \seeref{subsection:exploration_4_missing_cut_problem}. During test fitting, the machined joints were found to be too tight that they cannot be assembled manually by hammering. Almost 80$\%$ of the joints have to be adjusted manually. 

The cause of this problem was not identified. However, it is speculated to be related to machining accuracy similar to the previous problem \seeref{subsection:exploration_4_inaccurate_screw_hole_problem}.
...
Since the construction in Exploration Round 3, the accuracy of the robot-side have been improved by calibration \seeref{subsection:exploration_3_rfl_robot_inaccuracy_and_calibration} and the accuracy on the stationary-side PA structure have also been improved by deformation-aware design principles \seeref{subsection:exploration_4_deformation_awareness_and_error_correction_by_triangulation} and the use of scaffoldings  \seeref{subsection:exploration_5_scaffolding_support_during_assembly}.
