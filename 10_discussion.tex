...
\begin{itemize}
	\item Create parametric models of beams and joints \textit{(see \ul{7.3.1 Parametric Polyline Lap Joints})}

	\item Create design validation algorithms \textit{(see \ul{6.3.4 Design Software Implementation in Rhino Python})}

\end{itemize}
	\item Formulate assembly logic as TAMP problems

\begin{itemize}
	\item Create task planning flowcharts \textit{(see \ul{6.3.5.1 Task Planning with Flowchart})}
...
\end{itemize}
	\item Create process visualisation and reporting tools for designers, engineers and operators. \textit{(see \ul{6.3.5.6 Process Visualization and Adjustment})}
...
\begin{itemize}
	...
	\item Create software for on-site operators to execute pre-planned robotic tasks, monitor progress, and allow certain reactions to on-site conditions. \textit{(see \ul{6.3.8 Standalone Process Execution Controller})}

	\item Design active and passive alignment correction mechanisms. \textit{(see \ul{7.3.14 Camera-Marker Alignment Correction System})}
...
\begin{itemize}
	\item Design temporary foundation \textit{(see \ul{7.3.10 Beam to Ground Connection})}

	\item Design temporary scaffolding positions \textit{(see \ul{8.3.2 Scaffolding Support During Assembly})}

	\item Decide grasp pose and assembly sequence \textit{(see \ul{6.3.5.6 Process Visualization and Adjustment})}
...
\begin{itemize}
	\item New joint type \textit{(see \ul{7.3.2 Parametric Non-Planar Lap Joints})}
...
\end{itemize}
	\item Create robotic programme \textit{(see \ul{6.3.5 Process Design Workflow} and \ul{8.3.5 Specifying Actions and Goals for TAMP with PDDLStream})}

	\item Create CNC fabrication data for timber components \textit{(see \ul{5.6.1 Execution Plan} and \ul{7.5.1 Execution Plan})}
...
\begin{itemize}
	\item Monitor the automatic system and handle unexpected problems \textit{(see \ul{7.5.7 Screwdriver Dislodge Incident} / \ul{7.5.6 Screwdriver Tip Misalignment})}

	\item Perform machine tending and designated manual tasks 

\begin{itemize}
	\item Sort incoming delivery and check for defects \textit{(see \ul{7.5.3 Missing Cut Problem})}
...
\multicolumn{2}{|p{6.83cm}|}{{\footnotesize 9 (3 Cartesian + 6 Robot Joints) Refer to \ul{5.2.2 RFL Robotic Platform}}} \\ 
\hline
\multicolumn{1}{|p{3.04cm}}{{\footnotesize Repeatability}} & 
\multicolumn{2}{|p{9.71cm}}{{\footnotesize Not very relevant. See Answer 5 of \ul{10.1 Response to Research Questions}}} & 
