...
\begin{equation} \label{eq:one_chain_deviation}
    [T_P] = [Z_{P1}][X_{P1}][Z_{P2}][X_{P2}] … [X_{P_{n-1}}][Z_{Pn}] 
\end{equation}
...
\begin{align} \label{eq:deviation_with_two_links}
    d &= d_P + d_Q \nonumber\\
      &= d_{P1} + d_{P2} + … + d_{PN} + d_{Q1} + d_{Q2} + … + dQN 
\end{align}
...
The probability of the deviation $F_D(d)$ can be computed using Equation \ref{eq:probability_with_all_links}:
